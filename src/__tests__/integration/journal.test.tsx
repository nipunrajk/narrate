...
describe('Journal Integration Tests', () => {
  ...
  const mockEntries: JournalEntry[] = [
    ...
    },
    {
      ...
      user_id: 'user-123',
      content:
        'Working on the journal app. Making good progress with the features.',
  ...
  describe('Journal Entry Creation and Saving', () => {
    it('creates and saves a new journal entry successfully', async () => {
      ...
      await waitFor(() => {
        expect(saveEntry).toHaveBeenCalledWith(
          'This is my new journal entry about testing.'
        );
  ...
  describe('Past Entries Retrieval and Display', () => {
    it('fetches and displays user entries correctly', async () => {
      ...
      expect(
        screen.getByText(
          'Today was a great day. I learned a lot about React testing.'
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
    ...
    it('sorts entries by date (most recent first)', async () => {
      ...
      const entryElements = screen.getAllByText(
        /Today was a great day|Working on the journal app/
      );
  ...
  describe('Weekly Summary Generation with AI', () => {
    it('generates weekly summary with sufficient entries', async () => {
      ...
      const weeklyEntries = Array.from({ length: 6 }, (_, i) => ({
        ...
        content: `Entry ${
          i + 1
        }: Today I worked on various tasks and learned new things.`,
        created_at: new Date(
      ...
      vi.mocked(generateWeeklySummary).mockResolvedValue({
        ...
        data: {
          summary:
            'This week you focused on learning and productivity. You made consistent progress on your projects.',
      ...
      const summaryButton = screen.getByRole('button', {
        name: /generate.*weekly summary/i,
      });
      ...
      await waitFor(() => {
        expect(
          screen.getByText('This week you focused on learning and productivity')
        ).toBeInTheDocument();
    ...
    it('prevents summary generation with insufficient entries', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/write at least 5 entries/i)
        ).toBeInTheDocument();
      });
  ...
  describe('Complete Journal Workflow Integration', () => {
    it('completes full journal workflow from entry to summary', async () => {
      ...
      vi.mocked(generateWeeklySummary).mockResolvedValue({
        ...
        data: {
          summary:
            'This week you made great progress on testing and development.',
      ...
      await user.type(
        textarea,
        'Today I completed the journal integration tests.'
      );
      await user.click(screen.getByRole('button', { name: 'Save Entry' }));
    ...
    it('handles offline scenarios gracefully', async () => {
      ...
      await user.type(
        screen.getByRole('textbox'),
        'This entry will fail due to network error.'
      );
      await user.click(screen.getByRole('button', { name: 'Save Entry' }));

      await waitFor(() => {
        expect(
          screen.getByText(/network error|failed to save/i)
        ).toBeInTheDocument();
      });
    });
